...
tf = 24;             % Tiempo de simulacion en segundos (s)
...
%Tramo 1 (v=1 m/s, w=0 rad/s) por 5s
%Tramo 2 (v=0 m/s, w=-pi/2 rad/s) por 1s
%Tramo 3 (v=1 m/s, w=0 rad/s) por 5s 
%Tramo 4 (v=0 m/s, w=-pi/2 rad/s) por 1s
%Tramo 5 (v=1 m/s, w=0 rad/s) por 5s
%Tramo 6 (v=0 m/s, w=-pi/2 rad/s) pos 1s
%Tramo 7 (v=1 m/s, w=0 rad/s) por 5s
%Tramo 8 (v=0 m/s, w=-pi/2 rad/s) por 1s

u = [ 1*ones(1,50), 0*ones(1,10), ...
      1*ones(1,50), 0*ones(1,10), ...
      1*ones(1,50), 0*ones(1,10), ...
      1*ones(1,50), 0*ones(1,11) ];

w = [ 0*ones(1,50), -pi/2*ones(1,10), ...
      0*ones(1,50), -pi/2*ones(1,10), ...
      0*ones(1,50), -pi/2*ones(1,10), ...
      0*ones(1,50), -pi/2*ones(1,11) ];
